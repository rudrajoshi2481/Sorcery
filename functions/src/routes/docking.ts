...
router.post("/getoneproject", (req: any, res: any) => {
...
// router.post("/getallprojectslist",(req:any,res:any) => {
//   console.log("😊 /getallprojectslis ",req.body);

//   const {uuid} = req.body

//   const data = ProjectSchema.find({
//     createdBy:uuid
//   },
//   //  (err:any, docs:any) => {
//   //    if(err){
...
router.post("/getallprojectslist", (req: any, res: any) => {
  console.log("DOMWTHING ASDADSFASD");
  ...
  ProjectSchema.find({
    ...
    .then((docs) => {
      console.log(docs);
      res.send({ status: "200", msg: "fetched all projects", docs: docs });
    })
    .catch((err) => {
      res.send({ status: "400", msg: "no projects found" });
    });
...
router.post("/updatedockingproject", (req: any, res: any) => {
  // get input and save it in database
});
